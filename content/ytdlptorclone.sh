...
if [ "${RCLONE_AUTO_MODE}" = "move" ]; then
    ...
    rclone rc --user "${GLOBAL_USER}" --pass "${GLOBAL_PASSWORD}" --rc-addr=localhost:61802 operations/movefile srcFs="${FILEPATH}" srcRemote="${FILENAME}" dstFs="${REMOTE_PATH}" dstRemote="${FILENAME}" _async=true
    EXIT_CODE=$?
    ...
    rclone rc --user "${GLOBAL_USER}" --pass "${GLOBAL_PASSWORD}" --rc-addr=localhost:61802 operations/copyfile srcFs="${FILEPATH}" srcRemote="${FILENAME}" dstFs="${REMOTE_PATH}" dstRemote="${FILENAME}" _async=true
    EXIT_CODE=$?
    if [ ${EXIT_CODE} -eq 0 ]; then
        echo "[INFO] Successfully send job to rclone: $1 -> ${REMOTE_PATH}"
    else
